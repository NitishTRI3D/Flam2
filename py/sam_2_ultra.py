...
class LS_SAM2_ULTRA:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "image": ("IMAGE",),
                "bboxes": ("BBOXES", {"default": [[[235, 318, 506, 599], [212, 574, 472, 1022]]]}),
                'top_wear': ("STRING",),
                'bottom_wear': ("STRING",),
    ...
    RETURN_NAMES = ("first_mask", "second_mask")
    FUNCTION = 'sam2_ultra'
    CATEGORY = '😺dzNodes/LayerMask'

    def sam2_ultra(self, bboxes, image, top_wear, bottom_wear, sam2_model, precision,
        ...
        ret_first_masks = []
        ret_second_masks = []
        # bboxes = [[[235, 318, 506, 599], [212, 574, 472, 1022]]]

        top_wear_list = top_wear.split(',')
        bottom_wear_list = bottom_wear.split(',')
        ...
        for index in range(len(image)):
            ...
            print(len(bboxes[index]), len(top_wear_list), len(bottom_wear_list))
            # Handle possible bboxes
            if len(bboxes[index]) == 0 or len(top_wear_list) + len(bottom_wear_list) != len(bboxes[index]):
                ...
                ret_first_masks.append(image2mask(_mask))
                ret_second_masks.append(image2mask(_mask))
                continue
            else:
                boxes_np_batch = []
            ...
            with torch.autocast(mm.get_autocast_device(device), dtype=dtype) if autocast_condition else nullcontext():
                ...
                for i in range(len(image_np)):
                    ...
                    if out_masks.ndim == 3:
                        ...
                        first_mask_list.append(np.expand_dims(out_masks, axis=0))
                        second_mask_list.append(np.expand_dims(out_masks, axis=0))
                    else:
                        _, _, H, W = out_masks.shape
                        # Combine masks for all object IDs in the frame
                        # combined_mask = np.zeros((H, W), dtype=bool)
                        first_mask = np.zeros((H, W), dtype=bool)
                        second_mask = np.zeros((H, W), dtype=bool)
                        cnt = 0
                        print(len(top_wear_list), len(bottom_wear_list), len(out_masks))
                        for out_mask in out_masks:
                            # combined_mask = np.logical_or(combined_mask, out_mask)
                            if cnt < len(top_wear_list):
                                first_mask = np.logical_or(first_mask, out_mask)
                            else:
                                second_mask = np.logical_or(second_mask, out_mask)
                            cnt+=1
                        # combined_mask = combined_mask.astype(np.uint8)
                        # first_mask = out_masks[0]
                        # second_mask = out_masks[1]
                        first_mask = first_mask.astype(np.uint8)
                        second_mask = second_mask.astype(np.uint8)
                        # mask_list.append(combined_mask)
                        first_mask_list.append(first_mask)
                        second_mask_list.append(second_mask)
            ...
            first_out_list = []
            second_out_list = []
            for first_mask in first_mask_list:
                first_mask_tensor = torch.from_numpy(first_mask)
                first_mask_tensor = first_mask_tensor.permute(1, 2, 0)
                first_mask_tensor = first_mask_tensor[:, :, 0]
                first_out_list.append(first_mask_tensor)
            first_mask_tensor = torch.stack(first_out_list, dim=0).cpu().float()
            _first_mask = first_mask_tensor.squeeze()
            for second_mask in second_mask_list:
                second_mask_tensor = torch.from_numpy(second_mask)
                second_mask_tensor = second_mask_tensor.permute(1, 2, 0)
                second_mask_tensor = second_mask_tensor[:, :, 0]
                second_out_list.append(second_mask_tensor)
            second_mask_tensor = torch.stack(second_out_list, dim=0).cpu().float()
            _second_mask = second_mask_tensor.squeeze()
            ...
            if process_detail:
                if detail_method == 'GuidedFilter':
                    ...
                    _first_mask = guided_filter_alpha(pil2tensor(orig_image), _first_mask, detail_range // 6 + 1)   
                    _first_mask = tensor2pil(histogram_remap(_first_mask, black_point, white_point))
                    _second_mask = guided_filter_alpha(pil2tensor(orig_image), _second_mask, detail_range // 6 + 1)
                    _second_mask = tensor2pil(histogram_remap(_second_mask, black_point, white_point))
                elif detail_method == 'PyMatting':
                    # _mask = tensor2pil(mask_edge_detail(pil2tensor(orig_image), _mask, detail_range // 8 + 1, black_point, white_point))
                    _first_mask = tensor2pil(mask_edge_detail(pil2tensor(orig_image), _first_mask, detail_range // 8 + 1, black_point, white_point))
                    _second_mask = tensor2pil(mask_edge_detail(pil2tensor(orig_image), _second_mask, detail_range // 8 + 1, black_point, white_point))
                else:
                    # _trimap = generate_VITMatte_trimap(_mask, detail_erode, detail_dilate)
                    _first_trimap = generate_VITMatte_trimap(_first_mask, detail_erode, detail_dilate)
                    _second_trimap = generate_VITMatte_trimap(_second_mask, detail_erode, detail_dilate)
                    # _mask = generate_VITMatte(orig_image, _trimap, local_files_only=local_files_only, device=device,
                                            #   max_megapixels=max_megapixels)
                    # _mask = tensor2pil(histogram_remap(pil2tensor(_mask), black_point, white_point))
                    _first_mask = generate_VITMatte(orig_image, _first_trimap, local_files_only=local_files_only, device=device,
                                              max_megapixels=max_megapixels)
                    _first_mask = tensor2pil(histogram_remap(pil2tensor(_first_mask), black_point, white_point))
                    _second_mask = generate_VITMatte(orig_image, _second_trimap, local_files_only=local_files_only, device=device,
                                              max_megapixels=max_megapixels)
                    _second_mask = tensor2pil(histogram_remap(pil2tensor(_second_mask), black_point, white_point))
            else:
                # _mask = tensor2pil(_mask)
                _first_mask = tensor2pil(_first_mask)
                _second_mask = tensor2pil(_second_mask)

            # ret_image = RGB2RGBA(orig_image, _mask.convert('L'))
            # ret_images.append(pil2tensor(ret_image))
            # ret_masks.append(image2mask(_mask))
            ret_first_masks.append(image2mask(_first_mask))
            ret_second_masks.append(image2mask(_second_mask))
        ...
        return (torch.cat(ret_first_masks, dim=0), torch.cat(ret_second_masks, dim=0))
